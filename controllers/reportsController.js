...
async create(req, res, next) {
    try{
      ...
      return res.status(201).json({
        message: 'Se creo una categoria nueva de reportes',
        success: true,
        data: data.id
      });
      ...
      console.log(`Se produjo un error en el método de crear categoria de reportes en reportsController, Error: ${error}`);
    return res.status(501).json({
      message: 'Se produjo un error en el método de crear categoria de reportes en reportsController',
      success: false,
      error: error
    });
    }
